...
class TaskController extends Controller
{
    ...
    public function getTaskById($taskId)
    {
        ...
        // Fetch roles for all members in the task
        $memberRoles = DB::table('project_role_user')
            ->where('team_id', $task->team_id)
            ->whereIn('user_id', $task->members->pluck('id'))
        ...
        $formattedTask = [
            ...
            'members' => $task->members->map(function ($member) use ($memberRoles) {
                $roleId = $memberRoles->get($member->id)?->role_id ?? null;
                $role = $roleId ? Role::find($roleId)->name : 'member'; // Default to 'member' if role not found
                ...
                return [
                    ...
                    'email' => $member->email,
                    'color' => $this->getMemberColor($member->id),
                    'role' => $role, // Include the member's role
                ];
            }),
